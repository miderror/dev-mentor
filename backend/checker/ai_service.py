...
def _get_prompt_for_wrong_answer(
    ...
    system_prompt = "Ты - умный отладчик кода на Python. Ты СТРОГО следуешь формату. Ты НЕ добавляешь лишних слов. Ты отвечаешь ТОЛЬКО на русском."
...
async def get_ai_suggestion(check: Check, task: Task) -> tuple[str, int]:
    ...
    if check.status == Check.Status.SUCCESS:
        prompt = _get_prompt_for_success(task.description, check.code)

    elif check.status == Check.Status.ERROR:
        if check.stderr:
            prompt = _get_prompt_for_runtime_error(check.code, check.stderr)
        elif check.error_context:
            context = check.error_context
            prompt = _get_prompt_for_wrong_answer(
                task.description,
                check.code,
