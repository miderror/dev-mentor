...
def _get_prompt_for_success(task_description: str, user_code: str) -> tuple[str, str]:
    ...
    part_1 = dedent("""
        ИНСТРУКЦИЯ: Проанализируй КОД и ОПИСАНИЕ ЗАДАЧИ КОТОРУЮ РЕШАЕТ КОД после ТВОЯ ЗАДАЧА. Предоставь ТОЛЬКО ТВОЙ ВЫВОД в формате Markdown, точно следуя ШАБЛОНУ ВЫВОДА из ПРИМЕР.
...
def _get_prompt_for_runtime_error(
    ...
    part_1 = dedent("""
        ИНСТРУКЦИЯ: Проанализируй КОД и ОШИБКУ после ТВОЯ ЗАДАЧА. Предоставь ТОЛЬКО ТВОЙ ВЫВОД в формате Markdown, точно следуя ШАБЛОНУ ВЫВОДА из ПРИМЕР.
...
def _get_prompt_for_wrong_answer(
    ...
    part_1 = dedent("""
        ИНСТРУКЦИЯ: Проанализируй КОД и ОПИСАНИЕ ЗАДАЧИ КОТОРУЮ ДОЛЖЕН РЕШАТЬ КОД и ДЕТАЛИ ПРОВАЛЕННОГО ТЕСТА после ТВОЯ ЗАДАЧА. Предоставь ТОЛЬКО ТВОЙ ВЫВОД в формате Markdown, точно следуя ШАБЛОНУ ВЫВОДА из ПРИМЕР.
